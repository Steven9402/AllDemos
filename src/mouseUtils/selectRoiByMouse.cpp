...
void myfdrawLine(cv::Mat& img, vector<Point> polygon)
{
    if(polygon.size()>1){
        for(int idx=0;idx<polygon.size()-1;idx++){
            line(img, polygon[idx], polygon[idx+1], Scalar(0, 255, 0), 2);
        }
        line(img, polygon[0], polygon[polygon.size()-1], Scalar(0, 255, 0), 2);
    }
}
